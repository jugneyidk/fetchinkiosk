...
| --- | --- | --- | --- | --- |
| Extra allowed domains | Some web apps load sibling API/CDN domains | First-run setup allows URL host and true subdomains only | Add setup UI for extra hosts if needed | Before release if real app requires it |
...
| PIN configuration source | Admin escape security | First-run setup stores PBKDF2 material in private app data | Keystore-backed storage or remote validation for hardened deployments | Before production hardening |
